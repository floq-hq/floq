...
const { saveSettings, loadSettings, clearSettings } = vi.hoisted(() => ({
  saveSettings: vi.fn(),
  loadSettings: vi.fn((): Settings => ({ backgroundPolicy: 'forgiving', telemetryConsent: false, breakReminderEnabled: true, sessionStartReminderEnabled: true })),
  clearSettings: vi.fn(),
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  loadSettings.mockReturnValue({ backgroundPolicy: 'forgiving', telemetryConsent: false, breakReminderEnabled: true, sessionStartReminderEnabled: true });
  useSettingsStore.setState({ settings: { backgroundPolicy: 'forgiving', telemetryConsent: false, breakReminderEnabled: true, sessionStartReminderEnabled: true }, hydrated: false });
});
...
describe('setBackgroundPolicy', () => {
  it('updates the policy and persists it', () => {
    ...
    expect(saveSettings).toHaveBeenCalledWith({ backgroundPolicy: 'strict', telemetryConsent: false, breakReminderEnabled: true, sessionStartReminderEnabled: true });
  });
});

describe('hydrate', () => {
  it('loads persisted settings and flags hydrated', () => {
    loadSettings.mockReturnValue({ backgroundPolicy: 'strict', telemetryConsent: false, breakReminderEnabled: true, sessionStartReminderEnabled: true });
