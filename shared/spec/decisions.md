...
**Consent model:**
- `settings.telemetryConsent`, **default OFF** (opt-in, never opt-out). Lives in `useSettingsStore` / MMKV.
- **UPDATE 2026-05-31 (Mohamed, W6-sweep) — consent is now ACCOUNT-WIDE, not per-device** (supersedes the original "device-local, mirroring L15's backgroundPolicy"). It rides the `users/{uid}.settings` cross-device mirror (`useSettingsSync`, whole-blob LWW): turning it on/off on one device propagates to the user's other devices. Rationale: it's one user's account-level choice, like every other synced setting. The capture-vs-upload split below is unchanged — there is still **no backfill** of pre-consent local samples on any device; a device only egresses sessions it captured while the (now-synced) flag was ON.
- **Capture-vs-upload split (no-backfill now ENFORCED, W6-sweep):** the feature vector is captured into local SQLite on every session (on-device, L2-clean, unconditional). Each outbox row records `consented` = was consent ON **at capture time** (migration 005), and the egress flush ships only `consented = 1` rows. So **enabling consent later never backfills** sessions completed while it was off — critical now that consent syncs account-wide (a second device must not egress its own pre-consent outbox). *(Before this, the flush gated only on the current flag and would have uploaded the whole accumulated outbox on first enable — fixed.)*
